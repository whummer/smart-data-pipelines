...
var getProxyForURL = function(method, host, path, query, callback) {
	var result = {};
...
exports.logAndPermit = function(req, res) {
	...
	getProxyForURL(inv[HTTP_METHOD], u.host, u.pathname, u.query, function(result) {
...
var getIPInfo = function(ip, callback) {
	var IGNORED_IPS = ["127.0.0.1"];
	if(IGNORED_IPS.indexOf(ip) >= 0) {
		return callback({});
	}
