...
if [[ -z "$KAFKA_ADVERTISED_HOST_NAME" ]]; then
    export KAFKA_ADVERTISED_HOST_NAME=$(docker inspect --format '{{ .NetworkSettings.IPAddress }}' `hostname`)
...
#    TODO FR: when we have more than one ZK we need to revisit that
#    export KAFKA_ZOOKEEPER_CONNECT=$(env | grep ZOOKEEPER_.*PORT_2181_TCP= | sed -e 's|.*tcp://||' | paste -sd ,)
