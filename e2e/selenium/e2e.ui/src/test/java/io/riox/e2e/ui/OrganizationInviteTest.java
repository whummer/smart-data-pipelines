...
public class OrganizationInviteTest extends AbstractProxyTest {
	...
	public void testInvite() throws InterruptedException {
		...
		TestUtil.login(driver, TEST_USER1_EMAIL, TEST_USER1_PASS);

		/* USER1 INVITES USER2 */

		TestUtil.navToOrgsSettings(driver);

		WebElement orgLink = new WebDriverWait(driver, TIMEOUT_LONG)
					.until(ExpectedConditions.presenceOfElementLocated(By.linkText("Default Organization")));
		orgLink.click();

		WebElement newMemberField = new WebDriverWait(driver, TIMEOUT_LONG)
		...
		WebElement linkAccept = new WebDriverWait(driver, TIMEOUT_LONG)
